...
namespace DTcms.Common
{
    public class Utils
    {
        ...
        /// <param name="centSize">中间页码数量</param>
        /// <returns></returns>
        public static string OutwebPageList(int pageSize, int pageIndex, int totalCount, string linkUrl, int centSize)
        {
            //计算页数
            if (totalCount < 1 || pageSize < 1)
            {
            ...
            if (pageIndex <= 1)
            {
                firstBtn = "<a style=\"color:#999;\" >«上一页</a>";
            }
            if (pageIndex >= pageCount)
            {
                lastBtn = "<a style=\"color:#999;\" >下一页»</a>";
            }
            if (pageIndex == 1)
            {
            ...
            if (pageIndex == pageCount)
            {
                lastStr = "<a class=\"on\">" + pageCount.ToString() + "</a>";
            }
            int firstNum = pageIndex - (centSize / 2); //中间开始的页码
            if (pageIndex < centSize)
                firstNum = 2;
